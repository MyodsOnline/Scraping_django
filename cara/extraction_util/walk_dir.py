...
def scan_basedir() -> list:
    ...
    :return: список папок внутри директории или сообщение об ошибке, если путь не найден
    """
    try:
        first_level_dirs = []
...
@require_POST
@csrf_exempt
def get_fetch_response(request):
...
def scan_subdirectories(directory_name: str, file_name: str):
    """
    Ищет поддиректорию в указанной директории, которая совпадает с заданным временем.
    :param directory_name: Путь к директории для сканирования.
    :param file_name: Префикс, с которого должно начинаться название поддиректории.
    :return: Название поддиректории, если она найдена; иначе сообщение о том, что поддиректория не найдена.
    """
    ...
    if os.path.isdir(subdir_path):
        ...
        print(f'Директории {directory_name} нет в папке {BASE_PATH}')
